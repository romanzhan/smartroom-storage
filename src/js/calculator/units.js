...
export function initUnits({ container, onChange, unitsCatalog }) {
  const catalog = Array.isArray(unitsCatalog) && unitsCatalog.length
    ? unitsCatalog
    : unitsData;
  ...
  return {
    getSelectedUnit: () =>
      catalog.find((u) => u.id === selectedUnitId) || null,
  };
}
